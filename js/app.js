...
const SEED = {
  providers: [
    { id:'p1', name:'Maria Santos', role:'provider', email:'maria@demo.com', password:'123456',
      ...
      schedule:{ mon:true,tue:true,wed:true,thu:true,fri:true,sat:false,sun:false }, timePref:'morning',
      certs:[{ title:'Caregiver NC II', issuer:'TESDA', year:'2018' }, { title:'Basic Life Support', issuer:'Philippine Red Cross', year:'2023' }],
      portfolio:[{ title:'5 years live-in elderly care', desc:'Managed full-time care for a bedridden senior, including medication schedule and mobility assistance.' }] },
    { id:'p2', name:'Jose Reyes', role:'provider', email:'jose@demo.com', password:'123456',
      ...
      schedule:{ mon:true,tue:false,wed:true,thu:false,fri:true,sat:true,sun:false }, timePref:'afternoon',
      certs:[{ title:'Licensed Physical Therapist', issuer:'PRC', year:'2015' }],
      portfolio:[{ title:'Post-op rehab program', desc:'Designed a 6-week home rehab plan that restored unassisted walking for a knee-surgery patient.' }] },
    { id:'p3', name:'Luz Diaz', role:'provider', email:'luz@demo.com', password:'123456',
      ...
      schedule:{ mon:true,tue:true,wed:true,thu:true,fri:true,sat:true,sun:false }, timePref:'flexible',
      certs:[{ title:'Pediatric Nursing', issuer:'PRC', year:'2012' }, { title:'First Aid for Children', issuer:'Red Cross', year:'2022' }],
      portfolio:[{ title:'Newborn & toddler care', desc:'8 years supporting families with newborn care, feeding schedules, and early development.' }] },
    { id:'p4', name:'Roberto Cruz', role:'provider', email:'roberto@demo.com', password:'123456',
      ...
      schedule:{ mon:false,tue:true,wed:true,thu:true,fri:false,sat:true,sun:true }, timePref:'evening',
      certs:[{ title:'Registered Nurse', issuer:'PRC', year:'2010' }, { title:'ICU Critical Care', issuer:'Philippine Heart Center', year:'2014' }],
      portfolio:[{ title:'Post-appendectomy recovery care', desc:'Managed wound dressing and recovery monitoring for multiple post-surgical clients.' }] },
    { id:'p5', name:'Ana Villanueva', role:'provider', email:'ana@demo.com', password:'123456',
      ...
      schedule:{ mon:true,tue:true,wed:false,thu:true,fri:true,sat:false,sun:false }, timePref:'morning',
      certs:[{ title:'Home Management Training', issuer:'TESDA', year:'2019' }],
      portfolio:[{ title:'Full home organization', desc:'Specializes in deep-cleaning and household organization for busy families.' }] },
    { id:'p6', name:'Danny Gomez', role:'provider', email:'danny@demo.com', password:'123456',
      ...
      schedule:{ mon:true,tue:true,wed:true,thu:false,fri:false,sat:true,sun:true }, timePref:'flexible',
      certs:[{ title:'Dementia Care Specialist', issuer:'Alzheimer\'s Association PH', year:'2021' }],
      portfolio:[{ title:'Memory care support', desc:'Trained in redirection techniques and safety monitoring for dementia patients.' }] },
    { id:'p7', name:'Carla Mendoza', role:'provider', email:'carla@demo.com', password:'123456',
      ...
      schedule:{ mon:false,tue:true,wed:false,thu:true,fri:false,sat:true,sun:false }, timePref:'morning',
      certs:[{ title:'Diet & Nutrition Aide', issuer:'TESDA', year:'2020' }],
      portfolio:[{ title:'Diabetic meal planning', desc:'Plans and prepares weekly diabetic-friendly meals for elderly clients.' }] }
  ],
  users: [
    { id:'u1', name:'Carlos Client', role:'client', email:'client@demo.com', password:'123456', avatar:'CC', color:'#1565C0',
      schedulePref:{ days:{ mon:true,tue:true,wed:true,thu:true,fri:true,sat:false,sun:false }, timePref:'morning' } },
...
const TIME_PREF_LABELS = { morning:'Mornings', afternoon:'Afternoons', evening:'Evenings', flexible:'Flexible' };
...
function specLabel(p) { return (p && (p.specialtyLabel || TYPE_LABELS[p.specialty])) || 'General Caregiving'; }
function scheduleStrip(schedule) {
  if (!schedule) return '<span style="font-size:11px;color:var(--text-hint);">Schedule not set</span>';
  return DAYS.map(d => `<span class="sched-dot ${schedule[d] ? 'on' : ''}" title="${DAY_LABELS[d]}">${DAY_LABELS[d][0]}</span>`).join('');
}
  ...
  register() {
    ...
    if (role === 'provider') {
      const newProvider = { id, name, role, email, password: pass, specialty:'elderly', specialtyLabel:'Elderly Care',
        ...
        schedule:{ mon:false,tue:false,wed:false,thu:false,fri:false,sat:false,sun:false }, timePref:'flexible',
        certs:[], portfolio:[] };
      ...
      <div class="job-feed-list">
        ${provs.map(p => `
          <div class="job-feed-item">
            <div class="jfi-main">
              <div class="jfi-title">${p.name} — ${specLabel(p)}</div>
...
const hiring = {
    ...
    ui.openModal(`
      ...
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;">
        ...
        <div>
          ...
          <div class="jfi-sub">${specLabel(p)} · ₱${p.rate}/hr · ${starsHTML(p.rating, 11)} ${p.rating}</div>
        </div>
      </div>
  ...
  confirmHire(providerId) {
    ...
    const booking = {
      ...
      service: specLabel(p), date: dt.toISOString().slice(0, 10), time: dt.toTimeString().slice(0, 5),
...
const providers = {
  ...
  ensureScheduleFilterUI() {
    if (el('filter-day')) return;
    const bar = document.querySelector('#view-providers .filter-bar');
    const wrap = document.createElement('div');
    wrap.style.display = 'flex';
    wrap.style.gap = '8px';
    wrap.innerHTML = `
      <select id="filter-day" onchange="App.providers.filter()">
        <option value="any">Any Day</option>
        ${DAYS.map(d => `<option value="${d}">${DAY_LABELS[d]}</option>`).join('')}
      </select>
      <select id="filter-time" onchange="App.providers.filter()">
  ...
  render() {
    providers.ensureScheduleFilterUI();
    const filter = el('filter-specialization').value || providers.activeFilter;
    const day = el('filter-day') ? el('filter-day').value : 'any';
    const time = el('filter-time') ? el('filter-time').value : 'any';

    const list = Store.get('providers', []).filter(p => {
      if (filter !== 'all' && p.specialty !== filter) return false;
      if (day !== 'any' && !(p.schedule && p.schedule[day])) return false;
      if (time !== 'any' && p.timePref !== time && p.timePref !== 'flexible') return false;
      return true;
    });

    const box = el('providers-container');
    if (!list.length) {
      box.innerHTML = `<div class="empty-state"><i class="fa-solid fa-user-slash"></i><h4>No caregivers found</h4><p>Try a different specialization or schedule filter.</p></div>`;
      return;
    }
    box.innerHTML = list.map(p => `
      <div class="provider-card-row">
        ...
        <div class="jfi-main">
          ...
          <div class="jfi-sub">${specLabel(p)} · ${starsHTML(p.rating, 11)} ${p.rating} (${p.ratingCount})</div>
          <div class="jfi-sub">₱${p.rate}/hr · ${TIME_PREF_LABELS[p.timePref] || 'Flexible'}</div>
          <div class="sched-strip">${scheduleStrip(p.schedule)}</div>
        </div>
  ...
  renderProviderHome() {
    ...
    const clients = Store.get('users', []);
    const feed = el('provider-job-feed');
    feed.innerHTML = open.length ? open.map(r => {
      const client = clients.find(u => u.id === r.clientId);
      const pref = client && client.schedulePref;
      return `
      <div class="job-feed-item">
        <div class="jfi-main">
          ...
          <div class="jfi-sub">Client avail.: ${pref ? `${scheduleStrip(pref.days)} · ${TIME_PREF_LABELS[pref.timePref] || 'Flexible'}` : 'Not specified'}</div>
        </div>
        <button class="btn-small" onclick="App.providers.acceptJob('${r.id}')">Accept</button>
      </div>`;
    }).join('') : `<div class="empty-state"><i class="fa-solid fa-briefcase"></i><h4>No open postings</h4><p>Check back soon for new client requests.</p></div>`;
  },
...
const mapModule = {
  ...
  plot(list) {
    ...
    list.forEach(p => {
      ...
      marker.bindPopup(`
        ...
        <div class="popup-role">${specLabel(p)}</div>
  ...
  searchArea() {
    ...
    const filtered = Store.get('providers', []).filter(p =>
      p.name.toLowerCase().includes(q) || specLabel(p).toLowerCase().includes(q));
  ...
  openProfile(providerId) {
    ...
    ui.openModal(`
      <div style="text-align:center;margin-bottom:14px;">
        ...
        <h3>${p.name}</h3>
        <p style="font-size:12px;color:var(--text-secondary)">${specLabel(p)} · ₱${p.rate}/hr</p>
      </div>
      ...
      <p style="font-size:13px;color:var(--text-secondary);margin-bottom:14px;">${p.bio || 'No bio provided.'}</p>

      <div class="profile-subsection">
        <h4><i class="fa-solid fa-calendar-week"></i> Weekly Availability</h4>
        <div class="sched-strip lg">${scheduleStrip(p.schedule)}</div>
        <p class="profile-subsection-note">Preferred time: ${TIME_PREF_LABELS[p.timePref] || 'Flexible'}</p>
      </div>

      ${(p.certs && p.certs.length) ? `
      <div class="profile-subsection">
        <h4><i class="fa-solid fa-certificate"></i> Certifications & Training</h4>
        <div class="cert-list">
          ${p.certs.map(c => `<div class="cert-item"><i class="fa-solid fa-award"></i><div><strong>${c.title}</strong><span>${c.issuer}${c.year ? ' · ' + c.year : ''}</span></div></div>`).join('')}
        </div>
      </div>` : ''}

      ${(p.portfolio && p.portfolio.length) ? `
      <div class="profile-subsection">
        <h4><i class="fa-solid fa-briefcase"></i> Portfolio</h4>
        <div class="portfolio-list">
          ${p.portfolio.map(item => `<div class="portfolio-item"><strong>${item.title}</strong><p>${item.desc}</p></div>`).join('')}
        </div>
      </div>` : ''}

      <div class="profile-subsection">
        <h4><i class="fa-solid fa-star"></i> Reviews (${reviews.length})</h4>
...
const chatModule = {
  ...
  openChat(contactId) {
    ...
    el('chat-target-header').textContent = contact.name + (contact.role === 'provider' ? ` · ${specLabel(contact)}` : '');
...
const profile = {
  render() {
    ...
    el('profile-management-view').innerHTML = `
      ...
      <div class="menu-list">
        <div class="menu-item" onclick="App.profile.viewAllBookings()">
          <div class="menu-icon" style="background:#E3F2FD;color:#1565C0"><i class="fa-solid fa-calendar-check"></i></div>
          <span>${myBookingsCount} Total Bookings</span><i class="fa-solid fa-chevron-right"></i>
        </div>
        ${isProvider ? `
        <div class="menu-item" onclick="App.profile.viewMyReviews()">
          <div class="menu-icon" style="background:#FFF8E1;color:#F57C00"><i class="fa-solid fa-star"></i></div>
          <span>${u.rating || 0} Rating (${u.ratingCount || 0} reviews)</span><i class="fa-solid fa-chevron-right"></i>
        </div>` : ''}
      </div>
      ...
      <div class="menu-list">
        ...
        ${isProvider ? `
        <div class="menu-item" onclick="App.profile.managePortfolio()">
          <div class="menu-icon" style="background:#FCE4EC;color:#AD1457"><i class="fa-solid fa-briefcase"></i></div>
          <span>Portfolio & Certifications</span><i class="fa-solid fa-chevron-right"></i>
        </div>` : ''}
  ...
  editProfile() {
    ...
    ui.openModal(`
      ...
      <div class="form-group">
        <label>${isProvider ? 'Weekly Availability' : 'Preferred Care Schedule'}</label>
        <div class="day-checkbox-row">
          ${DAYS.map(d => {
            const checked = isProvider ? (u.schedule && u.schedule[d]) : (u.schedulePref && u.schedulePref.days && u.schedulePref.days[d]);
            return `<label class="day-checkbox"><input type="checkbox" data-day="${d}" class="sched-day-input" ${checked ? 'checked' : ''} /><span>${DAY_LABELS[d]}</span></label>`;
          }).join('')}
        </div>
      </div>
      <div class="form-group">
        <label>Preferred Time of Day</label>
        <select id="edit-timepref">
          ${Object.entries(TIME_PREF_LABELS).map(([val, label]) => {
            const current = isProvider ? u.timePref : (u.schedulePref && u.schedulePref.timePref);
  ...
  saveProfile() {
    ...
    const dayInputs = document.querySelectorAll('.sched-day-input');
    const days = {};
    dayInputs.forEach(inp => { days[inp.dataset.day] = inp.checked; });
    const timePref = el('edit-timepref').value;
    ...
    if (isProvider) {
      ...
      updates.schedule = days;
      updates.timePref = timePref;
    } else {
      updates.schedulePref = { days, timePref };
    }
  ...
  viewAllBookings() {
    const u = auth.currentUser;
    const isProvider = u.role === 'provider';
    const list = Store.get('bookings', []).filter(b => isProvider ? b.providerId === u.id : b.clientId === u.id)
    ...
    ui.openModal(`
      <h3><i class="fa-solid fa-calendar-check"></i> All Bookings (${list.length})</h3>
      <div class="booking-count-row">
        <div class="bc-pill confirmed">${counts.confirmed} Confirmed</div>
        <div class="bc-pill pending">${counts.pending} Pending</div>
        <div class="bc-pill completed">${counts.completed} Completed</div>
        <div class="bc-pill cancelled">${counts.cancelled} Cancelled</div>
      </div>
      <div class="bookings-list-wrapper" style="max-height:48vh;overflow-y:auto;margin-top:14px;">
        ${list.map(b => `
          <div class="booking-card">
            <div class="booking-card-header">
              <strong>${b.service}</strong>
            ...
            <div class="booking-info">
              <div class="booking-info-item"><i class="fa-solid fa-user-nurse"></i> ${isProvider ? (Store.get('users', []).find(x => x.id === b.clientId)?.name || 'Client') : b.providerName}</div>
              <div class="booking-info-item"><i class="fa-solid fa-calendar"></i> ${b.date} at ${b.time}</div>
            </div>
          </div>`).join('') || '<p style="font-size:12px;color:var(--text-hint);text-align:center;">No bookings yet.</p>'}
      </div>
    `);
  },
  ...
  viewMyReviews() {
    const u = auth.currentUser;
    const reviews = Store.get('ratings', []).filter(r => r.providerId === u.id)
      .sort((a, b) => new Date(b.date) - new Date(a.date));
    const clients = Store.get('users', []);

    ui.openModal(`
      <h3><i class="fa-solid fa-star"></i> My Reviews</h3>
      <div class="rating-summary">
        <div class="rating-big">${u.rating || 0}</div>
        <div class="rating-stars-wrap"><div class="stars">${starsHTML(u.rating || 0, 14)}</div><div class="rating-count">${u.ratingCount || 0} review(s)</div></div>
      </div>
      <div style="max-height:46vh;overflow-y:auto;display:flex;flex-direction:column;gap:8px;">
        ${reviews.map(r => {
          const client = clients.find(c => c.id === r.clientId);
          return `<div class="request-summary-item"><div class="rsi-main">
            <div class="rsi-title">${starsHTML(r.score, 11)} <span style="color:var(--text-hint);font-weight:400;">— ${client ? client.name : 'Client'}</span></div>
            <div class="rsi-sub">${r.comment}</div>
            <div class="rsi-sub">${r.date}</div>
          </div></div>`;
        }).join('') || '<p style="font-size:12px;color:var(--text-hint);text-align:center;">No reviews yet. Completed bookings will let clients rate you.</p>'}
      </div>
    `);
  },
  ...
  managePortfolio() {
    ...
    ui.openModal(`
      <h3><i class="fa-solid fa-briefcase"></i> Portfolio & Certifications</h3>

      <div class="profile-subsection">
        <h4><i class="fa-solid fa-certificate"></i> Certifications & Training</h4>
        <div id="manage-certs-list" class="cert-list">
          ${certs.map((c, i) => `
            <div class="cert-item">
              <i class="fa-solid fa-award"></i>
              <div><strong>${c.title}</strong><span>${c.issuer}${c.year ? ' · ' + c.year : ''}</span></div>
              <button class="remove-btn" onclick="App.profile.removeCert(${i})"><i class="fa-solid fa-trash"></i></button>
            </div>`).join('') || '<p style="font-size:12px;color:var(--text-hint)">No certifications added yet.</p>'}
        </div>
        <div class="grid-2-col" style="margin-top:10px;">
          <input type="text" id="new-cert-title" placeholder="Certificate / training title" />
          <input type="text" id="new-cert-issuer" placeholder="Issued by" />
        </div>
        <div class="grid-2-col" style="margin-top:8px;">
          <input type="text" id="new-cert-year" placeholder="Year (optional)" />
          <button class="btn-small" style="width:100%;justify-content:center;" onclick="App.profile.addCert()">+ Add Certification</button>
        </div>
      </div>

      <div class="profile-subsection">
        <h4><i class="fa-solid fa-images"></i> Portfolio</h4>
        <div id="manage-portfolio-list" class="portfolio-list">
          ${portfolio.map((p, i) => `
            <div class="portfolio-item">
              <div style="display:flex;justify-content:space-between;gap:8px;">
                <strong>${p.title}</strong>
                <button class="remove-btn" onclick="App.profile.removePortfolioItem(${i})"><i class="fa-solid fa-trash"></i></button>
              </div>
              <p>${p.desc}</p>
            </div>`).join('') || '<p style="font-size:12px;color:var(--text-hint)">No portfolio entries yet.</p>'}
        </div>
        <div class="form-group" style="margin-top:10px;">
          <input type="text" id="new-port-title" placeholder="Project / experience title" />
        </div>
        <div class="form-group">
          <textarea id="new-port-desc" rows="2" placeholder="Briefly describe this experience..."></textarea>
        </div>
        <button class="btn-small" style="width:100%;justify-content:center;" onclick="App.profile.addPortfolioItem()">+ Add Portfolio Entry</button>
      </div>

      <button class="btn-outline btn-full" style="margin-top:16px;" onclick="App.ui.closeModal()">Done</button>
    `);
  },

  addCert() {
    const title = el('new-cert-title').value.trim();
    const issuer = el('new-cert-issuer').value.trim();
    const year = el('new-cert-year').value.trim();
    if (!title || !issuer) { toast.show('Please provide both a title and issuer.', 'warning'); return; }
    const u = auth.currentUser;
    const certs = [...(u.certs || []), { title, issuer, year }];
    Store.update('providers', list => list.map(p => p.id === u.id ? { ...p, certs } : p), []);
    auth.currentUser = { ...u, certs };
    toast.show('Certification added.', 'success');
    profile.managePortfolio();
  },
  removeCert(i) {
    const u = auth.currentUser;
    const certs = (u.certs || []).filter((_, idx) => idx !== i);
    Store.update('providers', list => list.map(p => p.id === u.id ? { ...p, certs } : p), []);
    auth.currentUser = { ...u, certs };
    profile.managePortfolio();
  },
  addPortfolioItem() {
    const title = el('new-port-title').value.trim();
    const desc = el('new-port-desc').value.trim();
    if (!title || !desc) { toast.show('Please provide a title and description.', 'warning'); return; }
    const u = auth.currentUser;
    const portfolio = [...(u.portfolio || []), { title, desc }];
    Store.update('providers', list => list.map(p => p.id === u.id ? { ...p, portfolio } : p), []);
    auth.currentUser = { ...u, portfolio };
    toast.show('Portfolio entry added.', 'success');
    profile.managePortfolio();
  },
  removePortfolioItem(i) {
    const u = auth.currentUser;
    const portfolio = (u.portfolio || []).filter((_, idx) => idx !== i);
    Store.update('providers', list => list.map(p => p.id === u.id ? { ...p, portfolio } : p), []);
    auth.currentUser = { ...u, portfolio };
    profile.managePortfolio();
  }
